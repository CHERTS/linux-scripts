...
current_monit_ver=$(${monit_bin} -V | ${sed_bin} -n 1p | ${sed_bin} 's/[[:alpha:]|(|[:space:]]//g' | awk -F- '{print $1}')
...
if [ -f "${monit_tmp_dir}/monit-${monit_latest_ver}-${monit_platform}.tar.gz" ]; then
    ...
    echo -n "Unpacking monit-${monit_latest_ver}-${monit_platform}.tar.gz... "
    ...
    case "$(pidof monit | wc -w)" in
       ...
       *)  echo "Kill current monit... "
	   kill -9 $(pidof monit | awk '{print $1}')
	   ;;
    esac
    case "$(pidof monit | wc -w)" in
       0)  echo "Done"
