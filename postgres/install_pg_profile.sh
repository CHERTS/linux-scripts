...
if [ $? -eq 0 ]; then
	if [ -d "${SCRIPT_DIR}/pg_profile" ]; then
		...
		if [ $? -eq 0 ]; then
			...
			echo -e "${CYAN}*/30 * * * * ${PG_BIN_PATH}/psql -U postgres -d pg_profile -c 'SELECT profile.take_sample()' > /dev/null 2>&1${NC}"
			echo -e "${GREEN}For view sample execute:${NC}"
			echo -e "${CYAN}${PG_BIN_PATH}/psql -U postgres -d pg_profile -c 'SELECT profile.show_samples()'${NC}"
			echo -e "${GREEN}For create report execute:${NC}"
			echo -e "${CYAN}${PG_BIN_PATH}/psql -U postgres -d pg_profile -c  'SELECT profile.get_report(1, 11)' > report_1_11.html${NC}"
